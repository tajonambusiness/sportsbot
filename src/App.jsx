...
export default function App() {
  ...
  const [kalshiStatus, setKalshiStatus] = useState('FALLBACK')
  const [kalshiError, setKalshiError] = useState('Loading Kalshi markets...')
  const [marketsLoadedCount, setMarketsLoadedCount] = useState(0)
  const [weatherMarketsFoundCount, setWeatherMarketsFoundCount] = useState(0)

  useEffect(() => {
    let active = true
    async function loadMarkets() {
      try {
        setKalshiStatus('FALLBACK')
        setKalshiError('Loading Kalshi markets...')
        ...
        if (!active) return
        setMarketsLoadedCount(list.length)
        setWeatherMarketsFoundCount(weatherMarkets.length)
        ...
        if (weatherMarkets.length > 0) {
          ...
          setKalshiStatus('FALLBACK')
          setKalshiError('No live Kalshi weather markets found')
          setLastWeatherUpdate(new Date())
        }
      } catch (error) {
        if (!active) return
        setMarkets([FALLBACK_MARKET])
        setKalshiStatus('FALLBACK')
        setKalshiError(error?.message || 'Kalshi request failed')
        setLastWeatherUpdate(new Date())
      }
    }
    loadMarkets()
  ...
  const nextEdges = rankedMarkets.slice(1, 4).map((m) => ({
    market: m.title,
    platform: `${m.platform} • ${m.ticker} • ${m.locationName}`,
      ...
      <div className="mx-auto w-full max-w-md space-y-3 sm:space-y-4 md:max-w-3xl xl:max-w-5xl">
        ...
        <section className="space-y-3 rounded-2xl border border-electric/30 bg-gradient-to-b from-slate-900/95 to-slate-950 p-3.5 shadow-glow sm:space-y-4 sm:p-4 md:p-6">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400 md:text-base">{featured.platform} • {featured.ticker} • {featured.marketType} • {featured.locationName}</p>
            <p className={`mt-2 text-xs font-semibold ${kalshiStatus === 'LIVE' ? 'text-edge' : 'text-watch'}`}>Kalshi: {kalshiStatus}</p>
            {kalshiStatus === 'FALLBACK' && kalshiError && (
              <p className="mt-1 text-[11px] text-watch">{kalshiError}</p>
            )}
            <p className="mt-1 text-[11px] text-slate-400">Markets loaded: {marketsLoadedCount} • Weather markets found: {weatherMarketsFoundCount}</p>
          </div>
        ...
        <section className="rounded-2xl border border-electric/20 bg-panel/80 p-3.5 sm:p-4 md:p-5">
          ...
          <div className="space-y-2">
            {kalshiStatus === 'FALLBACK' && kalshiError === 'No live Kalshi weather markets found' && (
              <p className="rounded-xl border border-watch/40 bg-watch/10 p-2.5 text-sm text-watch">No live Kalshi weather markets found</p>
            )}
            {nextEdges.map((row) => (
              <div key={row.market} className="rounded-xl border border-slate-800 bg-slate-900/70 p-2.5 sm:p-3">
                ...
                <div className="grid grid-cols-4 items-center gap-2 text-sm">
                  ...
                  <span>{row.price}</span>
                  <span className={row.edge.startsWith('+') ? 'font-semibold text-edge' : row.edge === 'Needs mapping' ? 'font-semibold text-watch' : 'font-semibold text-skip'}>{row.edge}</span>
